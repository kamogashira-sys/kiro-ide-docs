...
| **L3a** | **節を持つパッチ**（実測15件） | **L2 と同じ箇条書きで書く**（専用ページが無いだけで情報量は L2 相当。0.8.135 は33項目ある） |
| **L3b** | 要約1〜2文のパッチ（実測30件） | **公式の説明を概要列に記載する** |
| **L3c** | 説明なしのパッチ（**実測0件**） | 版番号＋日付のみ |

> ⚠️ **「パッチは版番号と日付だけ書けばよい」は誤りです。**
> **全45パッチに公式の説明文が実在します**（実測）。説明を省くと、
> **公式が公開している情報を意図的に落とすことになります。**
> `make check-kiro-ide-changelog` は「L3a は箇条書きがあること・L3b は概要列が非空であること」を検証するため、
> 説明を書かないと**チェックが失敗します**。

> **専用ページを持つビルドの重複回避**: 専用ページがある6ビルド（1.0.52 / 89 / 116 / 138 / 182 / 242）は、
> 系列ページのパッチ説明が**専用ページのタイトルと同一**です（実測）。
> 専用ページ側を L2 として書き、**系列のパッチ表では専用ページへリンクするだけ**にします
> （**0.9.40 のみ表現が異なる**ため専用ページ側の記述を採用）。
