...
### 2.6 検索結果へのread権限・ignoreルールの適用（1.0.337）

**ファイル検索・テキスト検索は、各結果をread権限とignoreルールに照らして評価します。** ignore対象または読み取り不可のファイルのパスと、一致する内容は結果に表示されません。対象をeditor searchまたはagent searchのいずれかに限定する公式根拠は確認できないため、本ページでは検索種別をそのまま記載します。
...
### 6.2 サードパーティ拡張機能の組み込みAPI利用（1.0.395）

Kiroは専用のextension hostで動作するため、サードパーティ拡張機能はGit APIなどの組み込みeditor APIへのアクセスを維持できます。公式はVue（Volar）とAWS Toolkitを例に挙げていますが、対応拡張の網羅性やAPI互換性を保証する記述は公開していません。

### 6.3 大きなエージェント応答時のextension host信頼性（1.0.395）

大きなエージェント応答時にextension hostがクラッシュし、進行中の作業を失い得る中断を減らしました。クラッシュが完全になくなったという保証ではありません。
